...
class Twitter:
    ...
    def scrap(username,browser_name):
        try:
            ...
            driver.close()
            driver.quit()
            file_path = 'C:/Users/SRI GANESH/Desktop/Freelance/tasks/TwitterScraping/data.json'
            with open('data.json', 'w') as fp:
                json.dump(profile_data, fp)
            # open(file_path, 'wb') as fp:
                #pickle.dump(profile_data, fp)
            #with open(file_path, 'w') as output_file:
            #    print(profile_data, file=output_file)
            return profile_data
        except Exception as ex:
            return {"error" : ex}
            driver.close()
